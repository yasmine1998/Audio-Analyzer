...
function connect(event) {
    ...
    ws = new WebSocket("ws://" + window.location.hostname + ":8000/items/" + itemId.value + "/ws");
...
function wsSender() {
    ...
    this.handlerInterval = setInterval(() => {
        ...
        if (audioData.length > 0) {
            data_float = int16ToFloat32BitPCM(floatTo16BitPCM(audioData))
            array_data = Object.values(data_float)
            ws.send(array_data)       
        }
...
const handleSuccess = function (stream) {
    ...
    processor.onaudioprocess = function (audioProcessingEvent) {
        ...
        let sourceAudioBuffer = audioProcessingEvent.inputBuffer;
